//! In-memory implementations of persistence traits for dev/testing.

use async_trait::async_trait;
use chrono::Utc;
use dashmap::DashMap;

use super::agent_configs::{AgentConfig, AgentConfigStore};
use super::sessions::{Session, SessionStore, Turn, TurnRole};

// ---------------------------------------------------------------------------
// InMemorySessionStore
// ---------------------------------------------------------------------------

/// In-memory session store backed by `DashMap`. Not durable — intended for
/// development and testing only.
pub struct InMemorySessionStore {
    sessions: DashMap<String, Session>,
    /// Turns keyed by session_id.
...
/// In-memory agent config store backed by `DashMap`. Not durable — intended
...
mod tests {
    ...
    async fn get_recent_sessions_filters_by_peer() {
        ...
        store
            ...
            .unwrap();
        store
            .create_session("chan-2", "peer-2", "cli")
            .await
            .unwrap();
        store
            .create_session("chan-3", "peer-1", "web")
            .await
            .unwrap();
