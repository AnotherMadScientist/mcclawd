...
///
/// All execution runs inside Docker containers — there is no host-mode fallback.
/// If Docker is unavailable, the task fails immediately.
async fn run_agent(
    state: AppState,
    task_id: TaskId,
    prompt: &str,
    ...
    // Docker sandbox is mandatory — no host fallback
    ...
    // Docker unavailable — fail the task
    let msg = "Docker required for agent execution. Start Docker and try again.".to_string();
...
// Host execution removed — all agent execution runs inside Docker containers.
// See git history for the original run_agent_host implementation.
